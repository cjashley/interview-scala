...
class OneFrameDummy[F[_]: Applicative] extends Algebra[F] {
  ...
  override def getRate(pair: Rate.Pair): F[Error Either Rate] =
  {
    ...
    val rateO = OneFrameDummy.ratesStore.get(ccyPair)
    val r = if (rateO.isEmpty) OneFrameDummy.rss.add(ccyPair) else {
...
object OneFrameDummy
{
  ...
  def add(ratesStoreSupplier: RatesStoreSupplier) { throw new RuntimeException("Not Implemented")}
}